...
function findVariableByName(name, cachedVariables) {
    return (cachedVariables || figma.variables.getLocalVariables()).find((variable) => variable.name === name);
}
...
function findOrCreateVariable(name, collection, type, cachedVariables) {
...
figma.ui.onmessage = (msg) => {
    ...
    if (msg.type === "loadProgram") {
        ...
        for (let i = 0; i < msg.compiled.data.length; i++) {
            const dataMemVar = cachedVariables.find((variable) => variable.name === `datamem_${i}`);
            if (!dataMemVar) {
                console.log(`Could not find variable datamem_${i} - is the program too large?`);
                break;
            }
            dataMemVar.setValueForMode(dataMode, msg.compiled.data[i]);
        }
        // Clear rest of data mem
        for (let i = msg.compiled.data.length; i < datacollection.variableIds.length; i++) {
            const dataMemVar = cachedVariables.find((variable) => variable.name === `datamem_${i}`);
            if (!dataMemVar) {
                break;
            }
        ...
        for (let i = 0; i < msg.compiled.dispatch.length; i++) {
            const dispatchMemVar = cachedVariables.find((variable) => variable.name === `dispatchmem_${i}`);
            if (!dispatchMemVar) {
                console.log(`Could not find variable dispatchmem_${i} - is the program too large?`);
                break;
            }
            dispatchMemVar.setValueForMode(dispatchMode, msg.compiled.dispatch[i]);
        }
        // clear rest of dispatch mem
        for (let i = msg.compiled.dispatch.length; i < dispatchcollection.variableIds.length; i++) {
            const dispatchMemVar = cachedVariables.find((variable) => variable.name === `dispatchmem_${i}`);
            if (!dispatchMemVar) {
                break;
            }
    ...
    if (msg.type === "buildVarSets") {
        ...
        for (let i = 0; i < msg.dispatchmemcount; i++) {
            ...
            findOrCreateVariable(`dispatchmem_${i}`, dispatchcollection.id, "FLOAT", cachedVariables);
        }
        ...
        for (let i = 0; i < msg.datamemcount; i++) {
            ...
            findOrCreateVariable(`datamem_${i}`, datacollection.id, "FLOAT", cachedVariables);
        }
    }
